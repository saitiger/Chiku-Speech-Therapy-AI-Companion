...
const ScenarioCard: React.FC<ScenarioCardProps> = ({ scenario, onSelect }) => {
  ...
  return (
    <Card 
      className={`card-hover cursor-pointer w-full max-w-sm overflow-hidden ${scenario.backgroundClass}`}
      onClick={handleCardClick}
    >
      <CardContent className="p-6 flex flex-col items-center text-center">
        <div className="text-5xl mb-4 animate-bounce-slow">{scenario.iconName}</div>
        <h3 className="text-xl font-bold mb-2 text-white">{scenario.title}</h3>
